...
class WinScreen : public GameState {
  ...
  public:
    ...
    void draw() {
        ...
        LCD.WriteAt(s.c_str(), center(s.size()*CHAR_WIDTH, LCD_WIDTH), 32+18);

        const char *enterYourInitials = "Enter your initials:";
        LCD.WriteAt(enterYourInitials, center(strlen(enterYourInitials)*CHAR_WIDTH, LCD_WIDTH), 32+3*18);
  ...
  private:
    ...
    static const int initialsWidth = 24;
    static const int initialsHeight = 24; 
    static const int initialsSpacing = 16; 
};